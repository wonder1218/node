const { adModel, categoryModel } = require('../model')
...
class adController {
    ...
    static async add(ctx) {
        ...
        await ctx.render('admin/adAdd', { data: data })
    }
    ...
    static async list(ctx) {
            ...
            let allResponse = await adModel.findAll({ where, include })
            ...
            let response = await adModel.findAll({
            ...
            await ctx.render('admin/adList', { data: response, totalPage, currentPage, next, prev, keyword })

        }
        // 删除文章
    static async del(ctx) {
            let id = ctx.params.id;
            await adModel.destroy({
                where: { id },
                // cascade: true,
            })
            ctx.redirect('/ad', { id: id })
        }
        // 更新文章
    static async edit(ctx) {
        let id = ctx.params.id;
        let dataone = await adModel.findOne({ where: { id } })
        let data = await categoryModel.findAll({})
        await ctx.render('admin/adEdit', { dataone: dataone, data: data })
    }
    ...
    static async doedit(ctx) {
        ...
        await adModel.update(params, { where: { id } })
        ctx.redirect('/ad');
